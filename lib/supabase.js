...
async function getMyChats(userId) {
  try {
    ...
    const baseSelect = `
      ...
      portatore_id,
      updated_at,
      ucmes:ucmes (content, school_code)
    `;
    ...
    // ~~~ 3bis. Filtro chat scolastiche per utenti non premium ~~~
    const { data: userRow, error: userErr2 } = await client
      .from('users')
      .select('has_subscription')
      .eq('id', userId)
      .single();
    if (userErr2) throw userErr2;
    const isPremium = userRow?.has_subscription === true || userRow?.has_subscription === 'true';
    if (!isPremium) {
      allChats = allChats.filter((row) => !row.ucmes?.school_code);
    }
...
async function getChatMessages(chatId, requesterId) {
  try {
    ...
    // Arricchisce ogni messaggio con sender_id basato su sender_type
    let portatoreUserId = null;
    if (chatRow.portatore_id) {
      const { data: portMap, error: portMapErr } = await getSupabaseClient()
        .from('portatori')
        .select('user_id')
        .eq('id', chatRow.portatore_id)
        .single();
      if (!portMapErr && portMap) {
        portatoreUserId = portMap.user_id;
      }
    }

    const mapped = (data || []).map((msg) => ({
      ...msg,
      sender_id: msg.sender_type === 'user' ? chatRow.user_id : portatoreUserId
    }));

...
async function sendMessage(chatId, senderId, senderType = null, text) {
  try {
    ...
    if (!resolvedType) {
      resolvedType = chatRow.user_id === senderId ? 'user' : (chatRow.portatore_id === senderId ? 'portatore' : null);
    }
    if (!resolvedType) {
      throw new Error('Non autorizzato');
    }
    ...
    if (
      (resolvedType === 'user' && chatRow.user_id !== senderId) ||
      (resolvedType === 'portatore' && chatRow.portatore_id !== senderId)
    ) {
      // Proviamo a risolvere senderId ↔ portatore_id
      if (resolvedType === 'portatore') {
        ...
        if (portatoreRow && portatoreRow.id === chatRow.portatore_id) {
          // Autorizzato via mapping portatore
        } else {
          throw new Error('Non autorizzato');
        }
      } else {

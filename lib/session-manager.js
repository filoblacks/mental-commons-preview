...
async function validateSession(token, req) {
  try {
    ...
    const { sessionId, userId, email: decodedEmail, deviceFingerprint } = decoded;
    
    // Verifica sessione nel storage (in-memory)
    let session = activeSessions.get(sessionId);
    
    // ---------------------------------------------------------------
    // SUPPORTO SERVERLESS 🔥
    // ---------------------------------------------------------------
    // In ambiente Serverless (Vercel Functions) lo storage in-memory
    // viene azzerato ad ogni invocazione.  Il token JWT rimane però
    // perfettamente valido: contiene userId, email, exp, ecc.
    // Se la sessione non è presente in memoria ma il JWT è valido e
    // NON è scaduto, ricostruiamo un oggetto sessione minimale e
    // procediamo comunque, evitando 401/500 ricorsivi su ogni call.
    // ---------------------------------------------------------------
    if (!session) {
      const nowSec = Math.floor(Date.now() / 1000);
      if (decoded.exp && decoded.exp > nowSec) {
        debug(`⚠️  Sessione non trovata in memoria (cold start / serverless). Uso payload JWT come fallback.`);
        session = {
          sessionId,
          userId,
          email: decodedEmail,
        ...
        debug(`❌ Session not found and token expired`);
    ...
    return {
      ...
      email: session.email || decodedEmail
    };
